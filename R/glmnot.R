  ...
  if (is.data.frame(newdata)) {
    ...
    for (column in all.vars(no_response_formula)) {
      ...
      if (!is.numeric(column_data)) {
        ...
        newdata[[column]] <- ifelse(
          column_data %in% object$xlev[[column]],
          column_data,
          reference_level
        )
      }
...
glmnot.default <- function(
  ...
  structure(list(model = m, formula = formula, xlev = xlev), class = 'glmnot')
}

#' @importFrom Matrix sparse.model.matrix
#' @import dplyr
glmnot.formula <- function(
  formula,
  data,
  family,
  alpha = 1,
  ...
  constant_factors_in_formula <- intersect(
    constant_factors,
    all.vars(final_formula)
  )
